...
def make_support_matrix():
    ...
    with open('./step_config.yml', 'r') as file:
    ...
    alltypes = {
        ...
        "datetime_col": [datetime(2017, 4, 2, 10, 1, 0), datetime(2018, 4, 2, 10, 1, 0), None],
        ...
        for step_dict in expanded_steps:
            ...
            try:
                if hasattr(step, '_fit_expr'):
                    for expr in step._fit_expr:
                        fit_sql = ibis.to_sql(expr, backend)
                        msg = fit_sql
                tranform_sql = ibis.to_sql(output, backend)
                msg += tranform_sql
                results.append(True)
            except Exception as e:
                    msg = str(e)
                    results.append(False)
        steps[backend] = [r for r in results]

    support_matrix = (
        pd.DataFrame(steps).assign(
